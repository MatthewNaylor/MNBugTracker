...
$(document).ready(function () {
    ...
    var url = "/Graphs/TicketsOnProjectByPriority";
    if ($(".flot-bar")[0]) {
        $.post(url).then(function (response) {
            //var ticks = response.Ticks
            $.plot($(".flot-bar"), [{
                label: response.BarData.label,
                data: response.BarData.data,
                bars: {
                    order: response.BarData.bars.order,
                    fillColor: response.BarData.bars.fillColor
                }
            }],
